...
RETURNS TABLE (_uid INT, _displayname TEXT, _email TEXT, _roles TEXT[], _already_created BOOLEAN) AS $$ DECLARE
    var_cohort TEXT;
    var_uid INT;
...
BEGIN
    ...
    IF var_uid IS NULL THEN
        ...
        END IF;
        INSERT INTO users (displayname, email) VALUES (arg_displayname, arg_email) RETURNING uid INTO var_uid;
    END IF;

    -- Create the role for the user
    INSERT INTO user_roles (uid, cohort, role) VALUES (var_uid, var_cohort, arg_role) ON CONFLICT DO NOTHING;
    GET DIAGNOSTICS var_affected_rows = ROW_COUNT;
    IF var_affected_rows = 0 THEN
        SELECT TRUE INTO var_already_created;
    END IF;

    SELECT ARRAY(SELECT role FROM user_roles WHERE uid = var_uid) INTO var_roles;

    RETURN QUERY SELECT var_uid, var_displayname, var_email, var_roles, var_already_created;
