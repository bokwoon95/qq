// Package qybootstrap generates the source files for qxgen in the project cmd/ directory
package main
...
func main() {
	directory := filepath.Join("cmd", "qxgen")
	directoryFlag := flag.String("directory", directory, "(optional) Directory to place main.go. Default is cmd/qxgen in the current directory")
	flag.Parse()
...
var qxgenTemplate = ` + "`" + `// Code generated by qxgen; DO NOT EDIT.
package {{$.PackageName}}
...
func writeTablesToFile(tables []Table, directory, file, packageName string) error {
	...
	defer f.Close()
	t, err := template.New("").Funcs(template.FuncMap{"uppercase": strings.ToUpper}).Parse(qxgenTemplate)
	if err != nil {
		return err
	}
	data := FileData{
